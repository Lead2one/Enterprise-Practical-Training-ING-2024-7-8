...
// }
//
// mute_test.addEventListener('click',pageName)
// // mute.onclick() = pageName()

function chooseModel(){
    let address = location.href;
    console.log(address)
    if (address.includes("food.html")){
        model_choice = 0;
        console.log(model_choice);
    }
    else if (address.includes("medical.html")){
        model_choice = 3;
        console.log(model_choice);
    }
    else if(address.includes("travel.html")){
        model_choice = 4
        console.log(model_choice);
    }
}

function chooseModelDish(modelN){
   model_choice = modelN;
   console.log(model_choice);
}
...
document.addEventListener('DOMContentLoaded', function() {
    chooseModel();
    console.log(model_choice)
    fetch('http://127.0.0.1:5001/set_model_choice', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ model_choice: model_choice })  // 假设 data.data 包含用户 ID
    })
    .catch(error => {
